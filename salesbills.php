...
if(isset($_REQUEST['deletebill'])){
    ...
    $conn->query($sql);
    $sql = "DELETE FROM ".$table_name."_description WHERE billno=".$_REQUEST['deletebill'];
    $conn->query($sql);
...
// Check connection
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            $sql3 = "
            INSERT INTO `".$table_name."_bill` (`srno`, `bill_no`, `buyer_name`, `buyer_address`, `buyer_gst`,`amount`, `cgst`, `sgst`, `other_charges`, `sell_date`, `creation_date`) VALUES 
            (NULL, '".$_POST['billno']."', '".$_POST['buyersname']."', '".$row['address']."', '".$row["gstno"]."', '".$_POST['amount']."', '".($_POST['vat']/2)."', '".($_POST['vat']/2)."',
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) { 
            ...
            <tr>
                ...
                <td><button class="btn btn-success" onclick=" window.open('gst_bill.php?billno=<?=$row['srno']?>', '', 'height: 610,width: 297') ">Print</button></td>
            </tr>
        <?php
        }
    } else {
    ...
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-body">
            <table class="table">
                ...
                <tbody>
                    <?php
                        $sql2="SELECT * FROM ".$table_name."_description WHERE billno=".$row['bill_no'];
                        ...
                        if($totalprice > 0){ ?>
                            ...
                            <tr>
								<td colspan="1"><b>CGST : <?=round(($row['cgst']/100) * ($totalprice + $row['other_charges']))?></b></td>
                                <td colspan="1"><b>SGST : <?=round(($row['sgst']/100) * ($totalprice + $row['other_charges']))?></b></td>
								<td colspan="2"><b>Other Charges : <?=$row['other_charges']?></b></td>
								<td colspan="2"><b>Grand Total : <?=round(($row['cgst']/100) * ($totalprice + $row['other_charges']) + ($row['sgst']/100) * ($totalprice + $row['other_charges'])) + $totalprice + $row['other_charges']?></b></td>
                            </tr>
                        <?php
                        }
                    ?>
            ...
            <div class="modal-body">
                <form method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="vat">GST in %age:</label>
