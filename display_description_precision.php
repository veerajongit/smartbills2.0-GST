...
// Check connection
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo $sql2 = "
            INSERT INTO `precision_description` (`srno`, `billno`, `item_name`, `quantity`, `weight`, `item_rate`, `labour`, `creation_date`, `hsn_sac_code`) VALUES 
            (NULL, '".$_REQUEST['billno']."', '".$_REQUEST['itemname']."', '".$_REQUEST['quantity']."', '".$_REQUEST['weight']."',
             '".$row['item_price']."', '".$row['item_labour']."', CURRENT_TIMESTAMP, '".$row["hsn_sac_code"]."');
            ";
            $conn->query($sql2);
        }
    }
